...
pub fn main() !void {
    ...
    //
    // try world.physics.bodies.append(try rigidbody.DiscBody.init(alloc, Vector2.init(8, 1), 0, 2.0, 1.0));

    _ = try world.physics.makeDiscBody(Vector2.init(2, 5), 2.0, 0.2);
    _ = try world.physics.makeDiscBody(Vector2.init(6, 1), 8.0, 0.4);
    ...
    const point2_gravity = try forcegenerator.PointGravity.init(alloc, 2.0, .{ .x = 8, .y = 4 });
