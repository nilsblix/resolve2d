...
pub fn setup(solver: *Solver) !void {
    ...
    opt.mu = 0.3;
    var h: root.EntityFactory.BodyHandle = undefined;
    ...
    opt.pos = MID;
    opt.pos.y = -5;
    h = try fac.makeRectangleBody(opt, .{ .width = 1000, .height = 10 });
    h.body_unwrap().static = true;
    ...
    h = try fac.makeRectangleBody(opt, .{ .width = 4.0, .height = 1.0 });

    // const max: f32 = 100.0;
    // const params = Constraint.Parameters{ .beta = 100, .power_max = max, .power_min = -max };
    // _ = try fac.makeDistanceJoint(params, h.id, .{}, nmath.add2(h.body_unwrap().props.pos, Vector2.init(0.001, 0)), 0.0);
    ...
    for (10..30) |x| {
        ...
        for (10..36) |y| {
            ...
            if (@mod(y, 2) == 0) {
                h = try fac.makeRectangleBody(opt, .{ .width = 1.0, .height = 1.0 });
                // if (@mod(x, 2) == 0) {
                //     _ = try fac.makeSingleLinkJoint(params, h.id, .{}, nmath.add2(h.body_unwrap().props.pos, Vector2.init(0.001, 0)), 0.0);
                // }
            } else {
