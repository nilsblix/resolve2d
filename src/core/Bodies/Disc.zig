...
const rigidbody_vtable = RigidBody.VTable{
    .deinit = Disc.deinit,
    .updateAABB = Disc.updateAABB,
    .isInside = Disc.isInside,
    .closestPoint = Disc.closestPoint,
    .getNormal = Disc.getNormal,
    .projectAlongAxis = Disc.projectAlongAxis,
    .identifyCollisionPoints = Disc.identifyCollisionPoints,
    .clipAgainstEdge = Disc.clipAgainstEdge,
};

pub fn init(alloc: Allocator, id: RigidBody.Id, pos: Vector2, angle: f32, mass: f32, mu: f32, radius: f32) !RigidBody {
    var self: *Disc = try alloc.create(Disc);
    self.radius = radius;
...
pub fn deinit(ptr: *anyopaque, alloc: Allocator) void {
    const self: *Disc = @ptrCast(@alignCast(ptr));
    alloc.destroy(self);
}

pub fn updateAABB(rigidself: *RigidBody) void {
    const self: *Disc = @ptrCast(@alignCast(rigidself.ptr));
...
pub fn identifyCollisionPoints(rigidself: *RigidBody, incident: *RigidBody, active_normal_iter: usize) [MANIFOLD_MAX_PTS]?CollisionPoint {
    ...
    const edge = Disc.getNormal(rigidself.ptr, rigidself.props, incident.*, active_normal_iter);
    var normal = edge.?.dir;
